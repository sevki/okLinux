...
fn generate_sysno_enum(f: &mut impl Write, syscalls: &[SyscallInfo]) -> io::Result<()> {
    writeln!(f, "/// Syscall numbers scraped from vmlinux.")?;
    writeln!(f, "#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]")?;
    writeln!(f, "#[repr(usize)]")?;
    writeln!(f, "#[allow(non_camel_case_types)]")?;
    writeln!(f, "pub enum Sysno {{")?;
    for syscall in syscalls {
        let name = translate_syscall(&syscall.name);
        writeln!(f, "    {} = {},", name, syscall.num)?;
    }
    writeln!(f, "}}")?;
    Ok(())
}
...
fn generate_rust(syscalls: &[SyscallInfo]) -> io::Result<()> {
    ...
    writeln!(&mut f, "use super::*;\n")?;
    generate_sysno_enum(&mut f, syscalls)?;
    writeln!(&mut f)?;
